<?php
/* -----------------------------------------------------------------------------------------
   $Id: product.php 12418 2019-11-18 08:04:56Z GTB $
...
class product {
  ...
  function buildDataArray(&$array, $image='thumbnail') {
    ...
    if(defined('COMMERZFINANZ_STATUS') && COMMERZFINANZ_STATUS == 'true') {
        if($this->getCashDiscount($array['products_id']) > 0 ) {
            $commerz_finanz = commerz_finanz($array['products_id'], $array['products_tax_class_id']);
        }
    }
    //EOF - Commerz / Consors
    ...
    $productDataAdds = array (
      ...
      'COMMERZFINANZ' => $commerz_finanz['price'],
    );
   ...
   * @return string
   */
  function getCashDiscount($id) {
      $cashDiscount_query = xtDBquery("SELECT products_cash_discount FROM  ".TABLE_PRODUCTS_FIELDS." WHERE products_id = '".$id."'");
      $cashDiscount = xtc_db_fetch_array($cashDiscount_query);
      return $cashDiscount['products_cash_discount'];
  }
}
?>