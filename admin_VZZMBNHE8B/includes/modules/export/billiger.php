...
define('MODULE_BILLIGER_SHIPPING_METHOD_DESC', 'Die Versandkosten basieren auf Preis oder Gewicht (kg) des Artikels.');
...
p.products_manufacturers_model,
        ...
        // csv schema / headline
        $schema = 'aid;name;price;link;shop_cat;brand;mpr;ean;desc;image;dlv_time;dlv_cost';
        $schema .= "\n";
        // parse data
        while ($products = xtc_db_fetch_array($export_query)) {
            $aid = $products['products_id'];
            $name = $this->cleanVars($products['products_name']);
            $price = $xtPrice->xtcGetPrice($products['products_id'], $format = false, 1, $products['products_tax_class_id'], '');
            ...
            $brand = xtc_catalog_href_link('product_info.php', xtc_product_link($products['products_id'], $products['products_name']).(!empty($_POST['campaign']) ? '&'.$_POST['campaign'] : ''));
            $shop_cat = $this->buildCAT($this->getCategoriesID($products['products_id']));
            $brand = $products['manufacturers_name'];
            $mpr = $products['products_manufacturers_model'];
            $ean = $products['products_ean'];
            $desc = substr($this->cleanVars($products['products_short_description']), 0, 5000);
            $image = ($products['products_image'] != '') ? HTTP_CATALOG_SERVER . DIR_WS_CATALOG_POPUP_IMAGES . $products['products_image'] : '';
            $dlv_time = $this->getShippingtimeName($products['products_shippingtime']);
            $dlv_cost_nr = $this->getShippingCost($products['products_weight']);
            $dlv_cost =  number_format($dlv_cost_nr, 2, ',', '');
            
            if(stristr($products['products_description'], 'KEINE EINZELBESTELLUNG M&Ouml;GLICH!') !== FALSE) {
                continue;
            }
            
            if(($price + $dlv_cost_nr) >= 00.01) {
                // add line
                $schema .= $aid . ";" . // id
                    $name . ";" . // name
                    number_format($price, 2, ',', '') . ";" . // preis
                    $link . ";" . // link;
                    substr($shop_cat, 0, strlen($shop_cat)-2) . ";" . // kategorie
                    $brand . ";" . // hersteller
                    $mpr . ";" . // hersteller Artikelnummer
                    $ean . ";" . // EAN
                    $desc. ";". // Beschreibung
                    $image. "; " . // Bild
                    $dlv_time. "; " . //lieferzeit
                    $dlv_cost. "; " .  // lieferkosten
                    "\n";
            }
        }
    ...
    function keys()
    {
        return array('MODULE_BILLIGER_STATUS', 'MODULE_BILLIGER_FILE');
    }
