<?php
/* -----------------------------------------------------------------------------------------
   $Id: checkout_payment.php 12467 2019-12-05 18:26:50Z GTB $
...
// Zusätzlicher Gaggenau-Text
$gaggenau = false;
foreach($order->products as $product) {
    if($product['manufacturers_id'] == '57') {
        $gaggenau = true;
    }
    else {
        $gaggenau = false;
...
$smarty->assign('SHOW_GAGGENAU_TEXT', $gaggenau);
...
// BOF - AGB zur 5-Jahres-Garantie
$smarty->assign('WARRANTYCONDITIONS', true);
$smarty->assign('WARRANTYCONDITIONS_checkbox', '<input type="checkbox" value="conditions_warranty" name="conditions_warranty" id="conditions_warranty" '.(isset($_GET['step']) && $_GET['step'] == 'step2' ? ' checked="checked"' : '').' />');
// EOF - AGB zur 5 JahresGarantie

// BOF Weitergabe personenbezogener Daten an Versanddienstleister
$smarty->assign('SHIPPINGCONDITIONS', true);
$smarty->assign('SHIPPINGCONDITIONS_checkbox', '<input type="checkbox" value="conditions_shipping" name="conditions_shipping" id="conditions_shipping" '.(isset($_GET['step']) && $_GET['step'] == 'step2' ? ' checked="checked"' : '').' />');
// EOF _- Weitergabe personenbezogener Daten an Versanddienstleister

// BOF Kontaktaufnahme zur Kundenbewertungsanfrage
$smarty->assign('CONTACTCONDITIONS', true);
$smarty->assign('CONTACTCONDITIONS_checkbox', '<input type="checkbox" value="conditions_contact" name="conditions_contact" id="conditions_contact" '.(isset($_GET['step']) && $_GET['step'] == 'step2' ? ' checked="checked"' : '').' />');
