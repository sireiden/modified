<?php
 /*-------------------------------------------------------------
   $Id: orders_update.php 12204 2019-09-28 08:08:39Z GTB $
  ...
  if ($order->info['orders_status'] != $status || $comments != '' || $email_preview) {
    ...
    if ($_POST['notify'] == 'on' || $email_preview) {
      ...
      // BOF Dynamische Anhänge //
      $path_to_attachement = array();
      if($_POST['notify_add_agb'] == 'on') {
          $path_to_attachement[] = DIR_FS_DOCUMENT_ROOT . 'media/Allgemeine_Geschaeftsbedingungen_ascasa.pdf';
      }
      if($_POST['notify_add_warranty'] == 'on') {
          $path_to_attachement[] = DIR_FS_DOCUMENT_ROOT . 'media/Allgemeine_Geschaeftsbedingungen_5-Jahres-Garantie.pdf';
      }
      if($_POST['notify_add_revocation'] == 'on') {
          $path_to_attachement[] = DIR_FS_DOCUMENT_ROOT . 'media/Widerrufsbelehrung_ascasa.pdf';
      }
      if($_POST['notify_add_battery'] == 'on') {
          $path_to_attachement[] = DIR_FS_DOCUMENT_ROOT . 'media/Hinweis_Altbatterieentsorgung.pdf';
      }
      
      if(empty($path_to_attachement)) $path_to_attachement = '';
      // EOF Dynamische Anhänge //
