...
/* -----------------------------------------------------------------------------------------
  ...
  if (basename($PHP_SELF) == FILENAME_ADVANCED_SEARCH_RESULT) {
    ...
    $default_sorting = 'ORDER BY case  when p.manufacturers_id = 66 then 1 when pd.products_name LIKE "%'.$keyword.'%" then 1 else 3  end';
    ...
    if ($keywords && $keywordcheck) {
      ...
      if (PRODUCT_LIST_FILTER == 'true') { 
        ...
        $where_str .= ((isset($_SESSION['filter_sorting'])) ? $_SESSION['filter_sorting'] : $default_sorting);
      }
    }
