<?php
  /*-------------------------------------------------------------
   $Id: orders.php 12222 2019-09-30 16:32:08Z GTB $
...
switch ($action) {
  ...
  case 'update_order':
    if(!empty($_POST['mail_template_title']) && !empty($_POST['comments'])){
        xtc_db_query("insert into ".TABLE_MAIL_TEMPLATES." (title, mail_text) values ('".xtc_db_input($_POST['mail_template_title'])."', '".xtc_db_prepare_input($_POST['comments'])."')");
    }
