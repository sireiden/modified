...
define('MODULE_CHECK24_TEXT_DESCRIPTION', 'Einfach mit wenigen Klicks alle Artikel samt Versandkosten f&uuml;r Check24 exportieren.');
define('MODULE_CHECK24_TEXT_TITLE', 'Check24 Export - CSV');
define('MODULE_CHECK24_FILE_TITLE' , '<hr />Dateiname');
define('MODULE_CHECK24_FILE_DESC' , 'Geben Sie einen Dateinamen ein, falls die Exportadatei am Server gespeichert werden soll.<br>(Verzeichnis export/)');
define('MODULE_CHECK24_STATUS_DESC', 'Modulstatus');
define('MODULE_CHECK24_STATUS_TITLE', 'Status');
define('MODULE_CHECK24_CURRENCY_TITLE', 'W&auml;hrung');
define('MODULE_CHECK24_CURRENCY_DESC', 'Welche W&auml;hrung soll exportiert werden?');
...
class check24 {
    ...
    {
        $this->code = 'check24';
        $this->title = MODULE_CHECK24_TEXT_TITLE;
        $this->description = MODULE_CHECK24_TEXT_DESCRIPTION;
        $this->sort_order = MODULE_CHECK24_SORT_ORDER;
        $this->enabled = ((MODULE_CHECK24_STATUS == 'True') ? true : false);
        $this->CAT = array();
        $this->PARENT = array();
    }
    ...
    function process($file)
    {
        ...
        $hersteller = '57, 66, 67, 73, 80, 82, 83, 84, 97, 112, 117, 127, 128, 129, 130, 131, 132, 133, 134, 135, 137, 138, 139, 140, 145';
        @xtc_set_time_limit(0);
        
        $file = $_POST['configuration']['MODULE_CHECK24_FILE'];
        ...
        while ($products = xtc_db_fetch_array($export_query)) {
        	...
        	if($products['products_shippingtime'] == '31') $products['products_shippingtime'] = 12;
        	elseif($products['products_shippingtime'] == '30') $products['products_shippingtime'] = 12;
        	elseif($products['products_shippingtime'] == '11') $products['products_shippingtime'] = 12;
        	elseif($products['products_shippingtime'] == '1')  $products['products_shippingtime'] = 12;
        	elseif($products['products_shippingtime'] == '14') $products['products_shippingtime'] = 12;
        	elseif($products['products_shippingtime'] == '34') $products['products_shippingtime'] = 24;
        	elseif($products['products_shippingtime'] == '12') $products['products_shippingtime'] = 24;
        	elseif($products['products_shippingtime'] == '18') $products['products_shippingtime'] = 24;
        	elseif($products['products_shippingtime'] == '35') $products['products_shippingtime'] = 24;
        	elseif($products['products_shippingtime'] == '25') $products['products_shippingtime'] = 12;
        	elseif($products['products_shippingtime'] == '32') $products['products_shippingtime'] = 12;
            ...
            $ProduktURL = xtc_catalog_href_link('product_info.php', xtc_product_link($products['products_id'], $products['products_name']).(!empty($_POST['campaign']) ? '&'.$_POST['campaign'] : ''));
            ...
            /* Skonto wird nicht mehr ber³cksichtigt, da guenstiger keine negativen Werte akzeptieret
            ...
            */
            ...
            $Nachnahme = number_format($shipping_cost, 2, ',', '0') ;
    ...
    function display()
    {
        ...
        return array('text' =>
            ...
            xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=check24')) . '');
    }
    // check
    function check()
    {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_CHECK24_STATUS'");
    ...
    function install()
    {
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_CHECK24_FILE', 'check24.csv',  '6', '1', '', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_CHECK24_STATUS', 'True',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
    ...
    function keys()
    {
        return array('MODULE_CHECK24_STATUS', 'MODULE_CHECK24_FILE');
    }
