<?php
/* --------------------------------------------------------------
  $Id: categories.php 12294 2019-10-23 09:15:59Z GTB $
...
class categories {
  ...
  function insert_category($categories_data, $dest_category_id, $action = 'insert') {
    ...
    foreach ($languages AS $lang) {
      ...
      $sql_data_array = array('categories_name' => xtc_db_prepare_input($categories_data['categories_name'][$lang['id']]),
          ...
          'categories_description' => xtc_db_prepare_input(remove_word_code($categories_data['categories_description'][$lang['id']])),
  ...
  function remove_product($product_id) {
    ...
    /* BOF Löschen der zusätzlichen Tabellen */
    xtc_db_query("DELETE FROM ".TABLE_PRODUCTS_TABS." WHERE products_id = '".(int)$product_id."'");
    xtc_db_query("DELETE FROM ".TABLE_PRODUCTS_TO_ATTACHMENTS." WHERE products_id = '".(int)$product_id."'");
    xtc_db_query("DELETE FROM ".TABLE_PRODUCTS_FIELDS." WHERE products_id = '".(int)$product_id."'");
    /* EOF Löschen der zusätzlichen Tabellen */
  }
  ...
  function insert_product($products_data, $dest_category_id, $action = 'insert') {
    ...
    foreach ($languages AS $lang) {
      ...
      $sql_data_array = array('products_name' => xtc_db_prepare_input($products_data['products_name'][$language_id]),
					          ...
					          'products_description' => xtc_db_prepare_input(remove_word_code($products_data['products_description'][$language_id])),
				              'products_short_description' => xtc_db_prepare_input(remove_word_code($products_data['products_short_description'][$language_id])),
    ...
    /* BOF individuelle Produkt-Tabs */
    $highest_tab_id = 0;
    $product_tab_id_query = xtc_db_query("SELECT tab_id FROM " . TABLE_PRODUCTS_TABS . " WHERE products_id = '".$products_id."'");
    while($product_tab_id = xtc_db_fetch_array($product_tab_id_query)) {
        $tab_id = $product_tab_id['tab_id'];
        $highest_tab_id = ($tab_id > $highest_tab_id ) ? $tab_id : $highest_tab_id;
        if (isset($_POST['tab_delete'][$tab_id]) && $_POST['tab_delete'][$tab_id] == 'on') {
            $manufacturers_query = xtc_db_query("DELETE
                                                 FROM ".TABLE_PRODUCTS_TABS."
                                                 WHERE tab_id = '".$tab_id."'
                                                  AND products_id = '".$products_id."'");
            continue;
        }
        ...
        $sql_data_array = array(
            'status' => xtc_db_prepare_input($_POST['tab_status'][$tab_id]),
            'sort_order' => xtc_db_prepare_input($_POST['tab_sort_order'][$tab_id]),
            'tab_title' => xtc_db_prepare_input($_POST['tab_title'][$tab_id]),
            'tab_text' => xtc_db_prepare_input(remove_word_code($_POST['tab_text'][$tab_id])),
        );
        xtc_db_perform(TABLE_PRODUCTS_TABS, $sql_data_array, 'update', "products_id = '" . $products_id . "' and tab_id = '" . $tab_id . "'");
    }
    
    if(!empty($_POST['tab_title'][$highest_tab_id+1]) || !empty($_POST['tab_title'][$highest_tab_id+1])) {
        $sql_data_array = array(
            'products_id' => $products_id,
            'tab_id' => $highest_tab_id+1,
            'status' => xtc_db_prepare_input($_POST['tab_status'][$highest_tab_id+1]),
            'sort_order' => xtc_db_prepare_input($_POST['tab_sort_order'][$highest_tab_id+1]),
            'tab_title' => xtc_db_prepare_input($_POST['tab_title'][$highest_tab_id+1]),
            'tab_text' => xtc_db_prepare_input(remove_word_code($_POST['tab_text'][$highest_tab_id+1])),
        );
        
        xtc_db_perform(TABLE_PRODUCTS_TABS, $sql_data_array);
    }
    /* EOF individuelle Produkt-Tabs */
    
    /* BOF Zubehörartikel */
    if(isset($_POST['product_attachment'])) {
        foreach($_POST['product_attachment'] as $attachment_id => $sort) {
            $sql_data_array = array(
                'sort' => xtc_db_prepare_input($sort),
            );
            xtc_db_perform(TABLE_PRODUCTS_TO_ATTACHMENTS, $sql_data_array, 'update', "products_id = '" . $products_id . "' and attachment_id = '" . xtc_db_prepare_input($attachment_id) . "'");
        }
    }
    if(!empty($_POST['attachment_ids'])) {
        $current_result = xtc_db_query("SELECT MAX(sort) as current FROM ".TABLE_PRODUCTS_TO_ATTACHMENTS." WHERE products_id = '".$products_id."'");
        $current = xtc_db_fetch_array($current_result);
        $next = $current['current'] + 1;
        foreach($_POST['attachment_ids'] as $attachment_id) {
            if(!empty($attachment_id)) {
                xtc_db_query("INSERT INTO products_to_attachments
                              SET products_id   = '".$products_id."',
                              attachment_id = '".xtc_db_prepare_input($attachment_id)."',
                              sort = '".xtc_db_prepare_input($next)."'");
                $next++;
            }
    ...
    //BOF Einfügung für bisher sofort lieferbare Artikel
    if($_POST['shipping_status_old'] == 13 && $_POST['shipping_status'] != 13) {
        $product_categories_query = xtc_db_query("select categories_id from ".TABLE_PRODUCTS_TO_CATEGORIES." where products_id = '".$products_id."'");
        if (xtc_db_num_rows($product_categories_query)) {
            while ($product_categories = xtc_db_fetch_array($product_categories_query)) {
                if($product_categories['categories_id'] == 977) {
                    xtc_db_query("delete from ".TABLE_PRODUCTS_TO_CATEGORIES." where products_id = '".$products_id."' and categories_id = '".$product_categories['categories_id']."'");
                }
                else {
                    $categorie_to_check = $product_categories['categories_id'];
                    while($categorie_to_check != 0) {
                        $parent_query = xtc_db_query("select parent_id from ".TABLE_CATEGORIES." where categories_id = '".$categorie_to_check."'");
                        $parent = xtc_db_fetch_array($parent_query);
                        if($parent['parent_id'] == 977) {
                            xtc_db_query("delete from ".TABLE_PRODUCTS_TO_CATEGORIES." where products_id = '".$products_id."' and categories_id = '".$product_categories['categories_id']."'");
                        }
                        $categorie_to_check = $parent['parent_id'];
                    }
                }
            }
        }
    }
    //EOF Einfügung für bisher sofort lieferbare Artikel 	
    
    /* BOF Zusätzliche Artikelfelder */
    $products_manufacturers_pages = '';
    foreach($_POST['products_manufacturers_pages'] as $page_id => $status) {
        
        if($status == 1) {
            $products_manufacturers_pages .= $page_id.'|';
        }
    }
    
    $sql_data_array = array(
        'products_cash_discount' => xtc_db_prepare_input($_POST['products_cash_discount']),
        'products_efficiency_class' => xtc_db_prepare_input($_POST['products_efficiency_class']),
        'products_shippinginfo' => xtc_db_prepare_input($_POST['products_shippinginfo']),
        'products_best_price' => xtc_db_prepare_input($_POST['products_best_price']),
        'products_specialrequest' => xtc_db_prepare_input($_POST['products_specialrequest']),
        'products_manufacturers_pages' => $products_manufacturers_pages,
        'products_5year_warranty' => xtc_db_prepare_input($_POST['products_5year_warranty']),
        'products_warranty_conditions' => xtc_db_prepare_input($_POST['products_warranty_conditions']),
    );
    if ($action == 'insert') {
        ...
        $product_query = xtc_db_query("SELECT *
                                         FROM ".TABLE_PRODUCTS_FIELDS."
                                        WHERE products_id = '".$products_id."'");
        if (xtc_db_num_rows($product_query) == 0) {
            xtc_db_perform(TABLE_PRODUCTS_FIELDS, array('products_id' => $products_id));
        }
        xtc_db_perform(TABLE_PRODUCTS_FIELDS, $sql_data_array, 'update', "products_id = '".(int)$products_id."'");
    }
    /* EOF Zusätzliche Artikelfelder */
  ...
  function duplicate_product($src_products_id, $dest_categories_id) {
    ...
    // BOF - Produkt-Tabs ebenfalls duplizieren
    $product_tabs_copy_query = xtc_db_query("SELECT *
                                            FROM " . TABLE_PRODUCTS_TABS . "
                                           WHERE products_id = '".$src_products_id."'");
    while ($product_tabs_copy_data = xtc_db_fetch_array($product_tabs_copy_query)) {
        $sql_data_array = $product_tabs_copy_data;
        $sql_data_array['products_id'] = $this->dup_products_id;
        //write attributes data to DB
        xtc_db_perform(TABLE_PRODUCTS_TABS, $sql_data_array);
    }
    // EOF - Produkt-Tabs duplizieren
    
    //BOF - Artikelzubehör ebenfalls duplizizieren
    $product_attachments_copy_query = xtc_db_query("SELECT *
                                            FROM " . TABLE_PRODUCTS_TO_ATTACHMENTS . "
                                           WHERE products_id = '".$src_products_id."'");
    while ($product_attachments_copy_data = xtc_db_fetch_array($product_attachments_copy_query)) {
        $sql_data_array = $product_attachments_copy_data;
        $sql_data_array['products_id'] = $this->dup_products_id;
        //write attributes data to DB
        xtc_db_perform(TABLE_PRODUCTS_TO_ATTACHMENTS, $sql_data_array);
    }
    //EOF - Artikelzubehör ebenfalls duplizizieren
    
    //BOF - Zusätzliche Artikelfelder duplizieren
    $product_fields_copy_query = xtc_db_query("SELECT *
                                            FROM " . TABLE_PRODUCTS_FIELDS . "
                                           WHERE products_id = '".$src_products_id."'");
    while ($product_fields_copy_data = xtc_db_fetch_array($product_fields_copy_query)) {
        $sql_data_array = $product_fields_copy_data;
        $sql_data_array['products_id'] = $this->dup_products_id;
        //write attributes data to DB
        xtc_db_perform(TABLE_PRODUCTS_FIELDS, $sql_data_array);
    }
    //EOF - Zusätzliche Artikelfelder duplizieren 
