<?php
/* -----------------------------------------------------------------------------------------
   $Id: checkout_confirmation.php 12470 2019-12-09 13:17:25Z GTB $
...
/*  AGB zur 5-Jahres-Garantie */
if ((!isset($_POST['conditions_warranty']) || $_POST['conditions_warranty'] == false) && !isset($_GET['conditions_warranty'])) {
    $error = str_replace('\n', '<br />', 'Sofern Sie allgemeinen Gesch%E4ftsbedingungen zur 5 Jahres Garantie nicht bestätigen, k%F6nnen wir Ihre Bestellung bedauerlicherweise nicht entgegennehmen!');
    xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode($error), 'SSL', true, false));
}
/*  AGB zur 5-Jahres-Garantie */
  
/*  BOF Weitergabe personenbezogener Daten an Versanddienstleister */
if ((!isset($_POST['conditions_shipping']) || $_POST['conditions_shipping'] == false) && !isset($_GET['conditions_shipping'])) {
    $error = str_replace('\n', '<br />', 'Sofern Sie die Weitergabe Ihrer E-Mail-Adresse bzw. Telefonnummer an Versanddienstleister zur Bestellabwicklung nicht akzeptieren, können wir Ihre Bestellung bedauerlicherweise nicht entgegennehmen!');
    xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode($error), 'SSL', true, false));
}
/*  EOF Weitergabe personenbezogener Daten an Versanddienstleister */

/*  BOF Kontaktaufnahme zur Kundenbewertungsanfrage */
if ((!isset($_POST['conditions_contact']) || $_POST['conditions_contact'] == false) && !isset($_GET['conditions_contact'])) {
    $_SESSION['contact_bewertung'] = 0;
}
else {
    $_SESSION['contact_bewertung'] = 1;
}
/*  EOF Kontaktaufnahme zur Kundenbewertungsanfrage */
...
if (DISPLAY_CONDITIONS_ON_CHECKOUT == 'true') {
  ...
  $smarty->assign('TEXT_AGB_CHECKOUT', sprintf(TEXT_AGB_CHECKOUT, $shop_content_link, $main->getContentLink(REVOCATION_ID, MORE_INFO,'SSL')));
}
