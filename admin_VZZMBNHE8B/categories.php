...
if (USE_WYSIWYG == 'true') {
	...
	if ($action == 'new_product' || $action == 'new_product_preview') {
	  ...
	  /* BOF - Editor für zusätzliche Tabs */
	  $highest_tab_id = 0;
	  $product_tab_ids = array();
	  $product_tab_id_query = xtc_db_query("SELECT tab_id FROM " . TABLE_PRODUCTS_TABS . " WHERE products_id = '".(int) $_GET['pID']."'");
	  while($product_tab_id = xtc_db_fetch_array($product_tab_id_query)) {
	      array_push($product_tab_ids, $product_tab_id['tab_id']);
	      $highest_tab_id = ($product_tab_id['tab_id'] > $highest_tab_id ) ? $product_tab_id['tab_id'] : $highest_tab_id;
	  }
	  foreach($product_tab_ids as $product_tab_id) {
	      echo xtc_wysiwyg('tab_text', 'de', $product_tab_id);
	  }
	  $new_product_tab_id = $highest_tab_id+1;
	  echo xtc_wysiwyg('tab_text', 'de', $new_product_tab_id);
	  /* EOF - Editor für zusätzliche Tabs */ 
