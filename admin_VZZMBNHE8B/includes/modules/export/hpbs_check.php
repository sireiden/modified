...
class hpbs_check
{
    ...
    public function __construct()
    {
        ...
        $this->code = 'hpbs_check';
        $this->title = MODULE_HPBS_CHECK_TEXT_TITLE;
        $this->description = MODULE_HPBS_CHECK_TEXT_DESCRIPTION;
        $this->sort_order = MODULE_HPBS_CHECK_SORT_ORDER;
        $this->enabled = ((MODULE_HPBS_CHECK_STATUS == 'True') ? true : false);
    ...
    {
        $payment_class = 'hpbs';
        $paystatus = MODULE_PAYMENT_HPBS_PROCESSED_STATUS_ID;
        $finishstatus = MODULE_PAYMENT_HPBS_FINISHED_STATUS_ID;
    ...
    $treeMonth = 34 * 24 * 60 * 60;
        ...
        .' padding:0; top:0; left:0; opacity: 0.9; -moz-opacity: 0.9; -khtml-opacity: 0.9; filter:alpha(opacity=90);'
        .' z-index: 1000; width: 100%; height: 100%;"></div>';
        ...
        if (!empty($res)) {
            ...
            if (count($xmlObject->Result->Transaction) > 0) {
                ...
                .' action="module_export.php?set=&action=edit&dowork=1&module=hpbs_check">' .$crlf;
          ...
          $i++;
                    ...
                    $out.= '<input type="hidden" name="book['
                        .$i.'][shortId]" value="'.$v->Identification->ShortID.'">'.$crlf;
                    ...
                    $out.=    $order['customers_id'].'&action=edit" target="_blank">';
                    $out.= $order['customers_name'].'</a></td>'.$crlf;
    ...
    {
        $out = array('text' => HPBS_IMAGE_EXPORT_TYPE.'<br>'.
      HPBS_IMAGE_EXPORT.'<br>'
      .xtc_button_link(
          BUTTON_REVIEW_APPROVE,
                    xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set']
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query(
                ...
                . " where configuration_key = 'MODULE_HPBS_CHECK_STATUS'"
            );
    ...
    public function install()
    {
        xtc_db_query("insert into " . TABLE_CONFIGURATION
            ...
            ." values ('MODULE_HPBS_CHECK_STATUS', 'True',  '6', '1',"
